...
| | Status |
|---|---|
| Code & store | ✅ Built and smoke-tested (8/8) |
| Deployment artifacts | ✅ Dockerfile, Render, Fly, Procfile |
| Legal pages | ✅ Privacy, Terms, Refund, Shipping |
| Anthropic API key | ✅ Set |
| Stripe (payments) | ❌ Needs account + keys |
| Supplier (fulfilment) | ❌ Needs CJ Dropshipping account + keys |
...
You'll get something like `a3f8d2e1c9b4...`. Save it — you'll paste it into `SECRET_KEY` in your `.env` and/or platform dashboard.
...
3. Verify your email.
4. Complete the business profile (you can fill in legal details after launch, but Stripe needs a country).

### 2b. Get your API keys
1. In the Stripe dashboard, click **Developers** (top right) → **API keys**.
2. You'll see two keys:
   - **Publishable key** — starts with `pk_test_...` (test) or `pk_live_...` (live). Used in the frontend checkout JS. Not needed in `.env`.
   - **Secret key** — starts with `sk_test_...` or `sk_live_...`. This is `STRIPE_API_KEY`.
3. Copy the **Secret key** (click "Reveal"). Start with `sk_test_...` while testing; swap for `sk_live_...` when ready to charge real cards.

### 2c. Set up a webhook
Stripe sends your server a webhook event when a payment succeeds. Without this, orders won't auto-confirm.

1. Stripe dashboard → **Developers** → **Webhooks** → **Add endpoint**.
2. Endpoint URL: `https://yourdomain.com/api/orders/webhook/stripe` (fill in your real domain once deployed — come back to this after Step 5).
3. Events to listen for: select **`payment_intent.succeeded`** and **`checkout.session.completed`**.
...
3. Dashboard → **Domains** → **Add Domain** → verify DNS records for your domain (takes ~5 min). Or use the Resend shared domain for testing.
...
Render reads `render.yaml` from the repo and provisions everything automatically: a web service, a background worker, and a persistent disk for the database.
...
5. Before clicking **Apply**, you must fill in the secret environment variables. Click each service and add:
   ...
   | `STORE_NAME` | Whatever you want to call your store |
...
[ ] Legal pages load:
      ...
      /pages/refund
      /pages/shipping
[ ] Agents are running (check /api/admin/agent-logs or orchestrator logs)
```
...
2. **Developers → API Keys** → reveal and copy the **live** secret key (`sk_live_...`).
3. **Developers → Webhooks** → create a new webhook for live mode (same URL as before). Copy the new `whsec_...` signing secret.
4. Update `STRIPE_API_KEY` and `STRIPE_WEBHOOK_SECRET` in your platform dashboard with the live values.
...
| Ordering | Every 5 min | Picks up new orders, places with supplier, tracks shipping |
...
# ── DATABASE ───────────────────────────────────────────
# Leave as-is for SQLite (fine for most stores). For Postgres:
# DATABASE_URL=postgresql+asyncpg://user:pass@host:5432/dbname
DATABASE_URL=sqlite+aiosqlite:///./dropshipping.db

# ── PAYMENTS ───────────────────────────────────────────
STRIPE_API_KEY=sk_live_...              # sk_test_... while testing
STRIPE_WEBHOOK_SECRET=whsec_...
...
| Domain | — | ~$10–15/year |
| **Total to launch** | **$0 (test mode)** | **~$20–45/month running** |
...
- Stripe keys not set, or webhook not configured. Check Step 2.
